...
**Abstract**  
The automated extraction of information from complex PDF documents in mission-critical applications requires a robust approach to mitigating hallucination-related errors and data corruption. Modern Large Language Models (LLMs) suffer from a fundamental **structural perception bottleneck**: standalone LLMs are linguistically robust but often lack spatial awareness of document geometries. We categorize existing challenges into **Resolution-Loss constraints** (inherent in generative multimodal VLMs) and **Layout Unawareness** (inherent in traditional heuristic OCR). To address this, we formalize a novel **Hybrid OCR-VLM Synchronization** strategy that operates on a "Dual-Stream" logic. By rigorously grounding generative visual summaries in deterministic OCR character sequences, we enable the model to navigate fine-grained data and complex spatial hierarchies while reducing the incidence of hallucination-related errors. Our experimental results on a high-complexity DocVQA benchmark demonstrate that this **Hybrid approach achieves higher ANLS scores** over standalone VLM baselines in dense tabular environments. This paper formalizes the synchronization methodology and analyzes the resulting accuracy-efficiency trade-offs, establishing a reliability-oriented evaluation framework for industrial Document AI.
...
Existing OCR-based systems preserve literal textual precision but frequently fail to maintain spatial document structure in complex layouts. Conversely, modern Vision-Language Models preserve high-level visual semantics but remain vulnerable to resolution-loss hallucinations and lack deterministic grounding for exact-match extraction tasks. While prior work has explored layout-aware transformers and multimodal document understanding, limited research has systematically evaluated the reliability trade-offs between OCR, VLM, and synchronized Hybrid architectures under high-complexity zero-shot conditions.
...
![PaddleOCR Architecture](../figures/diagrams/paddleocr_academic.png)
**Figure 2.1: PaddleOCR Advanced Multi-Stage Architecture (DBNet + SVTR)**

![Layout Detection Architecture](../figures/diagrams/layout_detection.png)
...
$$C = S_{ocr} \parallel S_{vlm} \quad (1.1)$$
where $S_{ocr}$ represents the literal transcribed sequence and $S_{vlm}$ represents the structural spatial summary. This ensures that the retrieval engine can access both fine-grained textual tokens and high-level visual attributes. 
...
The Normalized Levenshtein distance ($NL$) is calculated as:
...
- **Retrieval**: The engine algorithmically searches the stored index to find vectors mathematically similar to the user's query embedding.
...
$$\cos(\theta)=\frac{\mathbf{A}\cdot\mathbf{B}}{|\mathbf{A}||\mathbf{B}|} \quad (3.4)$$
...
![Dataset Heterogeneity](../figures/diagrams/dataset_samples.png)
...
### 4.2 Benchmark Dataset and Protocol Justification

The relatively low absolute ANLS and Exact Match scores reflect the difficulty of the evaluation protocol rather than model instability. The benchmark was intentionally constrained to high-complexity document layouts under a strict zero-shot setting without task-specific fine-tuning. Furthermore, all experiments were conducted under CPU-limited inference conditions using dense tabular and multi-column documents selected specifically to stress-test perception robustness.

The perception strategies were evaluated using a strict Zero-Shot protocol, meaning that the models received no prior training or layout-specific fine-tuning before the 50-document benchmark. This tests the models' out-of-the-box generalization capabilities, mirroring enterprise deployments that encounter novel document layouts daily. 
Furthermore, the 50-document subset intentionally biases towards high-complexity, dense tabular data and multi-column formats. The benchmark was executed on CPU-bound infrastructure without dedicated GPU acceleration, accurately reflecting the resource constraints of many administrative servers and measuring the latency penalties of deep-learning perception layers in unaccelerated environments. The benchmark intentionally prioritizes adversarial layout complexity over dataset scale, resulting in lower absolute accuracy values but providing a more rigorous evaluation of structural robustness under realistic enterprise conditions.
...
The comparative capabilities of the distinct models are visualized across multiple dimensions across Accuracy, Resource Efficiency, Memory, and Storage overheads.
...
![System Efficiency](../figures/plots/efficiency_comparison.png)
**Figure 5.2: System Latency and Throughput Inversion**

![Resource Footprint](../figures/plots/memory_comparison.png)
...
The intrinsic trade-off between perception quality and inference speed is mapped in Figure 6.1. This analysis isolates the performance delta achieved by the synchronization model in dense tabular environments, confirming that literal grounding via the dual-stream approach is essential for mitigating generative hallucinations. The data highlights how the Hybrid model—representing the peak of the Accuracy-Efficiency Frontier—delivers improved fidelity by synchronizing independent perception streams to ensure a more robust document reasoning capability.
![Accuracy Metrics](../figures/plots/accuracy_tradeoff.png)
...
## 7. Discussion and Reliability Sections

### 7.1 Reproducibility

All experiments were executed using fixed pipeline configurations, identical retrieval parameters, and consistent embedding settings across all benchmark runs. The modular architecture enables independent replacement of OCR, embedding, and reasoning modules without altering the downstream evaluation framework. The implementation code, benchmark configurations, and evaluation scripts will be maintained in a public GitHub repository to support transparency and reproducibility.
...
Future work will focus on latency optimization via GPU-accelerated asynchronous tensor processing and scaling the Hybrid pipeline across more extensive, dense tabular-specific corpora to push Document AI closer to improved operational reliability. Additionally, the investigation of natively layout-aware multimodal architectures such as LayoutLMv3, which inherently incorporate geometric bounding boxes into transformer embeddings, represents a promising path for future research.
