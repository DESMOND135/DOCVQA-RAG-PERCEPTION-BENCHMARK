...
## Evaluation Metrics
- **N**: Total size of evaluation corpus (50 document queries)
- **a_i**: Predicted answer text generated by the cognitive model
- **g_i**: Authoritative human-verified ground truth answer
- **s(a_i, g_i)**: Similarity score with 0.5 threshold (similarity < 0.5 is scored 0)
- **EM**: Exact Match (absolute binary character-level identity)
- **F1**: F1-Score (harmonic mean of token-level Precision P and Recall R)
- **L_i**: Latency (seconds) to process query i
- **T_p**: System throughput in query samples per second
